package com.adamatomic.Mode
{
	...
	public class MenuState extends FlxState
	{
		...
		override public function create():void
		{
			...
			gibs = new FlxEmitter(FlxG.width/2-50,60);
			gibs.setSize(100,30);
			gibs.setYSpeed(-200,-20);
			gibs.setRotation(-720,720);
			gibs.gravity = 100;
			gibs.makeParticles(ImgGibs,250,32,true,0);
			add(gibs);

			//the letters "mo"
			title1 = new FlxText(FlxG.width + 20,50,80,"mo");
			title1.size = 32;
			...
			//the letters "de"
			title2 = new FlxText(-60,50,80,"de");
...
//			FlxG.mouse.hide();
		...
		override public function update():void
		{
			...
			if(title2.x > title1.x + title1.width - 28)
			{
				//Once mo and de cross each other, fix their positions
				title2.x = title1.x + title1.width - 28;
				title1.velocity.x = 0;
				title2.velocity.x = 0;
				...
				var text:FlxText;
				text = new FlxText(FlxG.width/2-56,FlxG.height/4+39,110,"by Adam Atomic")
				text.alignment = "center";
				text.color = 0x3a5c39;
				add(text);
				
				var flixelButton:FlxButton = new FlxButton(FlxG.width/2-40,FlxG.height/4+53,"flixel.org",onFlixel);
				...
				var mikeButton:FlxButton = new FlxButton(dannyButton.x,dannyButton.y + 22,"port: flashgen",onMike);
				mikeButton.color = flixelButton.color;
				mikeButton.label.color = flixelButton.label.color;
				add(mikeButton);
				
				playButton = new FlxButton(flixelButton.x,flixelButton.y + 82,"TAP TO PLAY",onPlay);
				playButton.color = flixelButton.color;
				playButton.label.color = flixelButton.label.color;
				add(playButton);
			}

			//if we sat on the menu too long, launch the attract mode instead!
			timer += FlxG.elapsed;
		...
		protected function onMike():void
		{
			FlxU.openURL("http://blog.flashgen.com");
		}
